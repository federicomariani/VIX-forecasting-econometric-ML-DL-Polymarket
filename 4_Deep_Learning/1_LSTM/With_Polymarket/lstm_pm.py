...
lstm_baseline = NeuralNetRegressor(
    module=LSTMModel,
    module__input_size=len(predittori), # da cambiare manualmente quando si utilizza predittori_w_pm
...
n_repeats = 10
importanza_media = np.zeros(len(predittori)) # da cambiare manualmente quando si utilizza predittori_w_pm
importanza_std = np.zeros(len(predittori)) # da cambiare manualmente quando si utilizza predittori_w_pm

for f in range(len(predittori)): # da cambiare manualmente quando si utilizza predittori_w_pm
    perdite = []
...
ranking = pd.DataFrame({
    "Variabile": predittori, # da cambiare manualmente quando si utilizza predittori_w_pm
...
lstm_wrapper = NeuralNetRegressor(
    module=LSTMModel,
    module__input_size=len(predittori), # anche qui da cambiare predittori usando la stessa metodologia
    max_epochs=50,
    **kwargs_comuni,
)
...
for i in range(n_test):
    ...
    X_new_seq = X_all_scaled[target_idx - lookback: target_idx, :].reshape(1, lookback, len(predittori)).astype(np.float32) # anche qui da cambiare predittori usando la stessa metodologia
