...
# -------------------------
dataset = pd.read_csv(
    r"C:\Users\fede1\Desktop\Repo\0_Dataset\Polymarket_data\dataset_w_polymarket.csv",
    index_col="Date",
    parse_dates=["Date"],
)
...
evento = "con_PM" # con e senza PM feature (senza_PM)
...
dataset["VIX_w"] = dataset["Close_VIX"].rolling(window=5).mean()
dataset["logVIX_MA_5"] = np.log(dataset["VIX_w"])

# Componente mensile
dataset["VIX_m"] = dataset["Close_VIX"].rolling(window=22).mean()
dataset["logVIX_MA_22"] = np.log(dataset["VIX_m"])
...
# Features Polymarket
pm_features = ["cut_50", "cut_25", "hold", "hike_25",
...
# Features tradizionali
features = ["logVIX_1", "logVIX_MA_5", "logVIX_MA_22"]

# Se scritto "con_PM" allora prende tutte le features, altrimenti solo quelle tradizionali
features = features + pm_features if evento == "con_PM" else features

# Elimina NaN generati dalle medie e dalle feature Polymarket
dataset = dataset.dropna(subset=features).reset_index()
...
def run_har_vix(h):
    ...
    # SALVA GRAFICO
    # --------------
    graph_path = (rf"C:\Users\fede1\Desktop\Repo\5_Forecasts_&_Error_Metrics\With_Polymarket\Modelli_econometrici\har_h{h}_{evento}.png")
...
# -----------------------------------
output_dir = r"C:\Users\fede1\Desktop\Repo\5_Forecasts_&_Error_Metrics\With_Polymarket\Modelli_econometrici"
...
for horizon in horizons:
    results[horizon] = run_har_vix(horizon)

results_h1 = results[1]
results_h5 = results[5]
results_h22 = results[22]
...
if not results_h1.empty:
    ...
    df_out_har_h1.to_csv(os.path.join(output_dir, f"har_h1_{evento}.csv"))
...
if not results_h5.empty:
    ...
    df_out_har_h5.to_csv(os.path.join(output_dir, f"har_h5_{evento}.csv"))
...
if not results_h22.empty:
    ...
    df_out_har_h22.to_csv(os.path.join(output_dir, f"har_h22_{evento}.csv"))
...
# summary.to_csv(os.path.join(output_dir, f"har_{evento}.csv"), index=False)
